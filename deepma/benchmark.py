...
Implements the split / normalization protocol from
``time_moe.datasets.benchmark_dataset.BenchmarkEvalDataset``:
...
def load_benchmark_split(
    ...
    sp = _SPLIT_IDX[split]
    if split == "train":
        # Use the full train segment [0, b2s[0]]; no context padding needed.
        seg = values[0:b2s[0]]
    else:
    ...
    # Vectorized window construction via stride tricks (no Python loop):
    #   produce (V, n_s - W + 1, W) views, then reshape into (V*N, W).
    ...
    # sliding_window_view returns a view; .copy() forces a contiguous block.
    W_view = sliding_window_view(series, window_shape=window_length, axis=1)
    # Shape (V, n_s - W + 1, W) -> (V*N, W); per-variable contiguous order.
